(()=>{
  ...
  const state={user:null,member:null,vehicles:[],ready:new Set(),space:null,tab:null,poll:null};
  ...
  function showLogin(message=""){$("staffLogin").hidden=false;$("staffApp").hidden=true;$("staffSignOut").hidden=true;$("staffLoginStatus").textContent=message;state.member=null;clearInterval(state.poll)}
  ...
  async function load(){
    ...
    $("staffLogin").hidden=true;$("staffApp").hidden=false;$("staffSignOut").hidden=false;
    await refresh();
  ...
  function select(space){state.space=space;state.tab=space.vehicleId?(isOwner()?'owner_vehicle':'sales_vehicle'):space.channel;renderSidebar();renderDetail()}
  ...
  async function renderMessages(channel,vehicleId){
    let request=db.from("staff_messages").select("id,author_id,author_name,body,created_at").eq("channel",channel).order("created_at",{ascending:false}).limit(200);
  ...
  async function sendMessage(event,channel,vehicleId){
    ...
    try{
      ...
      const row=await query(db.from('staff_messages').insert({channel,vehicle_id:vehicleId,author_id:state.user.id,author_name:state.member.display_name,body}).select('id').single());
      if(photo){
        ...
        await query(db.from('staff_attachments').insert({message_id:row.id,object_path:path,uploaded_by:state.user.id}));
      }
      await renderPane();
  ...
  $("staffSignOut").addEventListener('click',async()=>{await db.auth.signOut();state.user=null;showLogin()});
  ...
  $("staffTabs").addEventListener('click',event=>{const button=event.target.closest('[data-tab]');if(button){state.tab=button.dataset.tab;renderDetail()}});
