(()=>{
  ...
  async function init(){
    ...
    $('applicationSheet').innerHTML=`<header class="finance-application-sheet-head"><div><span class="eyebrow red">FINANCE APPLICATION</span><h1>${esc(name)}</h1><p>Submitted ${esc(datetime(row.created_at))} · Status: ${esc(row.status||'New')}</p></div><div class="finance-print-brand"><span class="finance-print-logo"><svg class="finance-print-logo-backing" viewBox="0 0 210 63" preserveAspectRatio="none" aria-hidden="true" focusable="false"><rect width="210" height="63" fill="#000"/></svg><img src="mo-motors-logo.png" alt="M.O Motors Certified Pre-Owned Vehicles"></span><span>Unit 104, 420 Des Meurons St<br>Winnipeg, MB R2H 2N9</span></div></header>${sections.map(s=>section(row,s)).join('')}<p class="finance-application-id">Application ID: ${esc(row.id)}</p>`;
